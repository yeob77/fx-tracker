...
const UsdPage = () => {
  ...
  const [usdSales, setUsdSales] = useLocalStorage<SaleRecord[]>('usdSales', []);
  ...
  const handleAddPurchase = (data: PurchaseData) => {
    if (data.id) { // Editing an existing purchase
      const originalLot = usdLots.find(lot => lot.id === data.id);
      ...
      setUsdLots(prevLots => [...prevLots, newLot].sort((a, b) => new Date(b.purchaseDate).getTime() - new Date(a.purchaseDate).getTime()));
    }
  ...
  const handleDeletePurchase = (lotToDelete: PurchaseLot) => {
    ...
    const hasAssociatedSales = usdSales.some(sale => sale.purchaseLotId === lotToDelete.id);
    ...
    setUsdLots(prevLots => prevLots.filter(lot => lot.id !== lotToDelete.id));
  };

  const handleSale = (data: SaleData) => {
    // Find the target purchase lot for the current sale (or edited sale)
    const targetLot = usdLots.find(lot => lot.id === data.purchaseLotId);
    if (!targetLot) return;

    let updatedUsdLots = [...usdLots];
    let updatedUsdSales = [...usdSales];
    ...
    if (data.id) { // Editing an existing sale
      ...
      updatedUsdSales = updatedUsdSales.map(sale => 
        sale.id === data.id 
      ...
      updatedUsdSales = [...usdSales, newSale].sort((a, b) => new Date(b.saleDate).getTime() - new Date(a.saleDate).getTime());
    ...
    setUsdLots(updatedUsdLots);
    setUsdSales(updatedUsdSales);
  ...
  const holdings = usdLots.filter(lot => lot.remainingQuantity > 0);
  const totalRealizedProfit = usdSales.reduce((sum, sale) => sum + sale.realizedProfit, 0);
  ...
  const filteredUsdSales = useMemo(() => {
    let filtered = usdSales;
    if (filterStartDate) {
    ...
    return filtered;
  }, [usdSales, filterStartDate, filterEndDate]);

  const sortedFilteredUsdSales = useMemo(() => {
    const sortableSales = [...filteredUsdSales];
  ...
  }, [filteredUsdSales, sortColumnSales, sortDirectionSales]);
    ...
    <div>
      ...
      {usdSales.length === 0 ? (
        ...
        <Table striped bordered hover responsive size="sm">
          ...
          <tbody>
            {sortedFilteredUsdSales.map(sale => {
              const purchaseLot = usdLots.find(lot => lot.id === sale.purchaseLotId);
              return (
                <tr key={sale.id}>
                  <td>{sale.saleDate}</td>
                  <td>{sale.quantity.toLocaleString()} USD</td>
